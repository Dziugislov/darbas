...
if effective_end_date != END_DATE:
    print(f"Warning: END_DATE '{END_DATE}' is in the future. Using today's date '{effective_end_date}' instead.")

print(f"Adjusted start date to {adjusted_start_date} to account for {JAW_MAX} days SMA calculation period")
print(f"Downloading {TICKER} data from {adjusted_start_date} to {effective_end_date}...")

# Download data with error handling
try:
    data = yf.download(TICKER, start=adjusted_start_date, end=effective_end_date)
    print(f"Downloaded data shape: {data.shape}")
    
    # Check if data is empty
    if data.empty:
        print(f"Error: No data downloaded for ticker '{TICKER}'. Trying with a different date range...")
        
        # Try with a more conservative end date (1 year ago)
        alternative_end = (datetime.today() - relativedelta(years=1)).strftime('%Y-%m-%d')
        print(f"Attempting download with end date: {alternative_end}")
        data = yf.download(TICKER, start=adjusted_start_date, end=alternative_end)
        
        if data.empty:
            print(f"Error: Still no data for '{TICKER}'. Please check the ticker symbol or try a different data source.")
            exit(1)
        else:
            print(f"Success! Downloaded data with alternative end date {alternative_end}")
            effective_end_date = alternative_end
...
print(f"Optimizing Alligator parameters...")
print(f"Jaw range: {JAW_MIN} to {JAW_MAX} with step {JAW_STEP}")
print(f"Lips ratio range: {LIPS_RATIO_MIN} to {LIPS_RATIO_MAX} with step {LIPS_RATIO_STEP}")
print(f"Teeth range: {TEETH_MIN} to {TEETH_MAX} with step {TEETH_STEP}")
...
if trading_start_idx >= len(data):
    print(f"Error: Simulation start date {SIMULATION_START_DATE} is after the latest available data.")
    print(f"Latest data date: {data.index[-1]}")
    print("Please adjust your SIMULATION_START_DATE in input.py.")
    exit(1)

trading_data = data.iloc[trading_start_idx:].copy()
if len(trading_data) < 100:  # Arbitrary minimum length
    print(f"Warning: Only {len(trading_data)} data points available for trading simulation.")
    print("This may not be enough for meaningful results.")
...
try:
    ...
    print(f"Error during Jaw-Lips optimization: {e}")
    # Use default values if optimization fails
    best_jaw_lips_params = (OPTIMAL_JAW, INITIAL_TEETH, OPTIMAL_LIPS)
    best_jaw_lips_sharpe = 0
    best_jaw_lips_trades = 0
    print(f"Using default parameters: Jaw = {best_jaw_lips_params[0]}, Lips = {best_jaw_lips_params[2]}")
...
try:
    ...
    print(f"Jaw = {best_params[0]} days, Teeth = {best_params[1]} days, Lips = {best_params[2]} days")
    print(f"In-sample Sharpe ratio = {best_sharpe:.4f}")
    print(f"Number of trades with optimal parameters = {best_trades}")
except Exception as e:
    print(f"Error during Teeth optimization: {e}")
    # Use default values if optimization fails
    best_params = (best_jaw, OPTIMAL_TEETH, best_lips)
    best_sharpe = 0
    best_trades = 0
    print(f"Using default parameters: Jaw = {best_params[0]}, Teeth = {best_params[1]}, Lips = {best_params[2]}")
...
try:
    ...
    plt.plot(results.index, results['Jaw_Strategy'], label=f'Jaw ({best_params[0]}-day SMA)', color='blue')
    plt.plot(results.index, results['Teeth_Strategy'], label=f'Teeth ({best_params[1]}-day SMA)', color='red')
    plt.plot(results.index, results['Lips_Strategy'], label=f'Lips ({best_params[2]}-day SMA)', color='green')
    ...
    plt.legend()
    plt.title(f'{TICKER} with Optimized Alligator Strategy ({best_params[0]}, {best_params[1]}, {best_params[2]}) - No Stop Loss')
    ...
    plt.legend()
    plt.title('Alligator Strategy Performance (Multiple of Initial Capital)')
    ...
    plt.tight_layout()
    output_file = os.path.join(OUTPUT_DIR, 'alligator_performance.png')
    ...
    try:
        ...
        print("\n--- PERFORMANCE SUMMARY (ALLIGATOR STRATEGY) ---")
        print(f"Simulation period: {SIMULATION_START_DATE} to {effective_end_date}")
        print(f"Alligator Parameters: Jaw={best_params[0]}, Teeth={best_params[1]}, Lips={best_params[2]}")
    ...
    print(f"Error during strategy application or visualization: {e}")
