"""
Alligator Strategy Hierarchical Analysis

This script performs hierarchical clustering analysis on Alligator strategy optimization results
...
def analyze_alligator_results(file_path=JAW_LIPS_RESULTS_FILE):
    """
    Analyze the Alligator optimization results and create a basic heatmap visualization
    """
    print(f"Loading Alligator optimization results from {file_path}...")
    ...
    print(f"\nBest parameters:")
    print(f"Jaw period: {best_jaw}")
    print(f"Teeth period: {best_teeth}")
    print(f"Lips period: {best_lips}")
    ...
    plt.title(f'Alligator Optimization Heatmap (Best Sharpe: {best_sharpe:.4f} at Jaw={best_jaw}/Lips={best_lips})',
              fontsize=14)
    ...
    plt.tight_layout()
    output_file = os.path.join(OUTPUT_DIR, 'alligator_heatmap.png')
...
def create_hierarchical_cluster_visualization(X, medoids, top_medoids, max_sharpe_point, labels):
    ...
    # Save figure
    output_file = os.path.join(OUTPUT_DIR, 'alligator_hierarchical_clusters.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
...
def hierarchical_cluster_analysis(file_path=JAW_LIPS_RESULTS_FILE, min_trades=MIN_TRADES, max_trades=MAX_TRADES,
    ...
    """
    Perform hierarchical clustering analysis on Alligator optimization results to find robust parameter regions
    ...
    """
    print(f"\n----- HIERARCHICAL CLUSTER ANALYSIS FOR ALLIGATOR STRATEGY -----")
    ...
    dendrogram_path = os.path.join(OUTPUT_DIR, 'alligator_hierarchical_dendrogram.png')
    ...
    # Print results
    print("\n----- HIERARCHICAL CLUSTERING RESULTS FOR ALLIGATOR STRATEGY -----")
    print(f"Max Sharpe point: Lips={int(max_sharpe_point[0])}, Jaw={int(max_sharpe_point[1])}, "
    ...
    for idx, medoid in enumerate(top_medoids, 1):
        print(f"Top {idx}: Lips={int(medoid[0])}, Jaw={int(medoid[1])}, "
              f"Sharpe={medoid[2]:.4f}, Trades={int(medoid[3])}")
...
def plot_alligator_performance(jaw_period, lips_period, teeth_period, top_medoids=None, slippage=SLIPPAGE, file_path=None):
    """
    Plot the Alligator strategy performance using the best parameters and include top medoids
    ...
    print(f"\n----- PLOTTING ALLIGATOR STRATEGY PERFORMANCE -----")
    print(f"Using Jaw Period: {jaw_period}, Teeth Period: {teeth_period}, Lips Period: {lips_period}")
    ...
    plt.tight_layout()
    output_file = os.path.join(OUTPUT_DIR, 'alligator_performance_with_medoids.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    ...
    if file_path is None:
        file_path = os.path.join(OUTPUT_DIR, 'alligator_performance_summary.txt')
        
    with open(file_path, 'w') as f:
        f.write("----- ALLIGATOR STRATEGY PERFORMANCE SUMMARY -----\n\n")
    ...
    csv_file = os.path.join(OUTPUT_DIR, 'alligator_performance_data.csv')
...
def bimonthly_out_of_sample_comparison(data, best_jaw, best_teeth, best_lips, top_medoid, slippage=SLIPPAGE):
    ...
    plt.tight_layout()
    output_file = os.path.join(OUTPUT_DIR, 'alligator_bimonthly_sharpe_comparison.png') 
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"\nBimonthly comparison chart saved as '{output_file}'")
    plt.show()
    
    # Save the bimonthly data to CSV
    csv_file = os.path.join(OUTPUT_DIR, 'alligator_bimonthly_sharpe_comparison.csv')
...
if __name__ == "__main__":
    ...
    # Plot strategy performance with the best parameters AND top medoids
    ...
    # Run bimonthly out-of-sample comparison if we have top medoids
