...
def main():
    ...
    def analyze_full_oos_performance_hierarchical(data, best_short_sma, best_long_sma, top_medoids,
                                          big_point_value=1, slippage=0, capital=1000000, atr_period=14):
        """
        Plot a comparison between the best strategy and a portfolio of top 3 hierarchical medoids.
        Only shows the P&L comparison plot without the price/SMA indicators.
        Writes TRUE/FALSE to column U (21) based on whether the portfolio outperforms the best strategy.
        ...
        plt.axhline(y=0.0, color='gray', linestyle='-', alpha=0.5, label='Break-even')
        plt.legend(loc='upper left')
        plt.title('Hierarchical Strategy Comparison: Best vs Portfolio of Medoids', fontsize=14)
        plt.ylabel('P&L ($)', fontsize=12)
        plt.grid(True, alpha=0.3)
        
        plt.tight_layout()
        save_plot('Hierarchical_Full_OOS_Performance_Analysis.png')
        ...
        try:
            ...
            print(f"Updating Excel file with hierarchical OOS comparison results for {SYMBOL}...")
            ...
            # Write TRUE/FALSE in column U (21) based on whether Portfolio outperforms Best
            sheet.cell(row=ticker_row, column=21).value = "TRUE" if portfolio_wins else "FALSE"

            # Save the workbook
            wb.save(excel_file)

            print(f"Excel file updated successfully. Added {SYMBOL} with hierarchical portfolio comparison {portfolio_wins} in column U")
