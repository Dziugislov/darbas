...
def main():
    ...
    def create_cluster_visualization(X_filtered_full, medoids, top_medoids, centroids, max_sharpe_point, labels=None, valid_clusters=None):
        ...
        plt.title(f'Unfiltered Heatmap (Only short_SMA < long_SMA)', pad=20)
        ...
        # Second plot: Trade-filtered heatmap (before clustering)
        print("Creating trade-filtered heatmap...")
        
        # Create filtered dataframe from the points that meet trade requirements
        filtered_by_trades_df = pd.DataFrame(X_filtered_full, columns=['short_SMA', 'long_SMA', 'sharpe_ratio', 'trades'])
        
        # Create a pivot table for the trade-filtered heatmap
        trades_filtered_heatmap = filtered_by_trades_df.pivot_table(
            index='long_SMA',
            columns='short_SMA',
            values='sharpe_ratio',
            fill_value=np.nan
        ...
        trades_mask = np.zeros_like(trades_filtered_heatmap, dtype=bool)
        for i, long_sma in enumerate(trades_filtered_heatmap.index):
            for j, short_sma in enumerate(trades_filtered_heatmap.columns):
                if short_sma >= long_sma or np.isnan(trades_filtered_heatmap.iloc[i, j]):
                    trades_mask[i, j] = True
        
        # Plot the trade-filtered heatmap
        plt.figure(figsize=(12, 10))
        ax = sns.heatmap(
            trades_filtered_heatmap,
            mask=trades_mask,
            cmap='coolwarm',
            annot=False,
            fmt='.4f',
        ...
        # Plot max Sharpe point on trade-filtered heatmap
        try:
            best_x_pos = np.where(trades_filtered_heatmap.columns == max_sharpe_point[0])[0][0] + 0.5
            best_y_pos = np.where(trades_filtered_heatmap.index == max_sharpe_point[1])[0][0] + 0.5
            plt.scatter(best_x_pos, best_y_pos, marker='*', color='lime', s=200,
                        edgecolor='black', zorder=5)
        except IndexError:
            print(f"Warning: Max Sharpe point at ({max_sharpe_point[0]}, {max_sharpe_point[1]}) not found in trade-filtered heatmap coordinates")
        
        plt.legend(handles=[max_sharpe_handle], loc='lower right')
        plt.title(f'Trade-Filtered Heatmap (MIN_TRADES to MAX_TRADES)', pad=20)
        plt.xlabel('Short SMA (days)', fontsize=12)
        plt.ylabel('Long SMA (days)', fontsize=12)
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        ...
        # Third plot: Final cluster visualization with all filters
        print("Creating cluster visualization with all filters...")
    ...
    def create_filtered_heatmap(data, min_trades=MIN_TRADES, max_trades=MAX_TRADES):
        """
        Create a heatmap visualization of Sharpe ratios filtered by trade count.
        This shows the same data as the regular heatmap but only for parameter combinations
        that result in a reasonable number of trades.
        """
        print(f"\nCreating filtered heatmap (trades between {min_trades} and {max_trades})...")

        # Filter data by number of trades
        filtered_data = data[
            (data['trades'] >= min_trades) &
            (data['trades'] <= max_trades)
        ].copy()

        print(f"Filtered from {len(data)} to {len(filtered_data)} parameter combinations")
        ...
        # Find the best Sharpe ratio in the filtered data
        best_idx = filtered_data['sharpe_ratio'].idxmax()
        best_short_sma = filtered_data.loc[best_idx, 'short_SMA']
        best_long_sma = filtered_data.loc[best_idx, 'long_SMA']
        best_sharpe = filtered_data.loc[best_idx, 'sharpe_ratio']
        best_trades = filtered_data.loc[best_idx, 'trades']
        ...
        # Set labels and title
        plt.title(f'Filtered SMA Optimization Heatmap ({min_trades}-{max_trades} trades)\n' +
                f'Best Sharpe: {best_sharpe:.4f} at {best_short_sma}/{best_long_sma} with {best_trades} trades',
                fontsize=14)
        plt.xlabel('Short SMA (days)', fontsize=12)
        plt.ylabel('Long SMA (days)', fontsize=12)
        ...
        # Display the plot
        plt.tight_layout()
        save_plot('Filtered_Heatmap.png')
    ...
    def analyze_full_oos_performance(data, best_short_sma, best_long_sma, top_medoids,
        ...
        plt.axhline(y=0.0, color='gray', linestyle='-', alpha=0.5, label='Break-even')
        plt.legend(loc='upper left')
        plt.title('Strategy Comparison: Best vs Portfolio of Medoids', fontsize=14)
        plt.ylabel('P&L ($)', fontsize=12)
        plt.grid(True, alpha=0.3)
        
        plt.tight_layout()
        save_plot('Full_OOS_Performance_Analysis.png')
        
        print(f"Full out-of-sample performance analysis plot saved to 'Full_OOS_Performance_Analysis.png'")
    ...
    # Create filtered heatmap before clustering
    filtered_data = create_filtered_heatmap(data)
