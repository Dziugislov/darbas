...
# Load the SMA simulation results - this function stays the same as in original code
...
# New function to compute medoids for hierarchical clustering
...
# New function to create dendrogram visualization for hierarchical clustering
def create_dendrogram(X_scaled, method='ward', figsize=(12, 8), color_threshold=None, truncate_mode=None,
                      p=10, save_path='dendrogram.png'):
    """
...
# New function to create cluster visualization for hierarchical clustering results
def create_hierarchical_cluster_visualization(X, medoids, top_medoids, max_sharpe_point, labels):
    ...
    # Create a new figure with 2 subplots (2x1)
...
# Modified function to perform hierarchical clustering analysis
def hierarchical_cluster_analysis(file_path='sma_all_results.txt', min_trades=MIN_TRADES, max_trades=MAX_TRADES,
    ...
    # Alternative: Could use automated methods to find optimal clusters
    # Example: from scipy.cluster.hierarchy import fcluster
    # k = len(np.unique(fcluster(Z, t=0.7*max(Z[:,2]), criterion='distance')))
    # print(f"Automatically determined {k} clusters based on dendrogram distance threshold")
...
# This function stays the same as in original code
