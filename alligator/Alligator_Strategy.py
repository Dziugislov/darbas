...
class AlligatorStrategy:
    """
    Classic Bill Williams' Alligator trading strategy using three SMAs with shifts
    
    Features:
    - Jaw (Blue): Longest SMA (slow), shifted 8 bars into the future
    - Teeth (Red): Medium SMA (medium), shifted 5 bars into the future
    - Lips (Green): Shortest SMA (fast), shifted 3 bars into the future
    
    Classic Entry Rules:
    - Long: When price crosses above the Alligator's mouth (Lips line) from below
           and the Alligator's mouth is open (Jaw, Teeth, Lips are properly ordered)
    - Short: When price crosses below the Alligator's mouth (Lips line) from above
            and the Alligator's mouth is open (Jaw, Teeth, Lips are properly ordered)
    
    Classic Exit Rules:
    - When price crosses back through the Lips line in the opposite direction
    - When the Alligator's mouth starts to close (lines converging)
    """

    def __init__(self, jaw_period, teeth_period, lips_period, contract_size, slippage=0):
        """
        Initialize the Classic Alligator strategy with specific parameters
        ...
        self.jaw_shift = 8  # Classic shift for Jaw
        self.teeth_shift = 5  # Classic shift for Teeth
        self.lips_shift = 3  # Classic shift for Lips
        self.contract_size = contract_size
        self.slippage = slippage

    def apply_strategy(self, data, strategy_name="Strategy", initial_capital=INITIAL_CAPITAL):
        """
        Apply the Classic Alligator strategy with entry and exit rules
        """
        # Create a copy of the DataFrame to avoid modifying the original
        sim_data = data.copy()

        # Calculate SMAs for the Alligator indicator (without shift initially)
        sim_data[f'Jaw_Base_{strategy_name}'] = sim_data['Close'].rolling(window=self.jaw_period).mean()
        sim_data[f'Teeth_Base_{strategy_name}'] = sim_data['Close'].rolling(window=self.teeth_period).mean()
        sim_data[f'Lips_Base_{strategy_name}'] = sim_data['Close'].rolling(window=self.lips_period).mean()
        
        # Apply the shifts (future shift in classic Alligator)
        # For a future shift, we need to shift rows upward (shift with negative value)
        sim_data[f'Jaw_{strategy_name}'] = sim_data[f'Jaw_Base_{strategy_name}'].shift(-self.jaw_shift)
        sim_data[f'Teeth_{strategy_name}'] = sim_data[f'Teeth_Base_{strategy_name}'].shift(-self.teeth_shift)
        sim_data[f'Lips_{strategy_name}'] = sim_data[f'Lips_Base_{strategy_name}'].shift(-self.lips_shift)
        ...
        for i in range(max(self.jaw_period, self.teeth_period, self.lips_period) + max(self.jaw_shift, self.teeth_shift, self.lips_shift), len(sim_data) - max(self.jaw_shift, self.teeth_shift, self.lips_shift)):
            ...
            prev_price = sim_data.iloc[i-1]['Close']
            prev_lips = sim_data.iloc[i-1][f'Lips_{strategy_name}']
            
            # Check if the Alligator's mouth is open properly
            # For long positions: Lips > Teeth > Jaw
            mouth_open_long = (curr_lips > curr_teeth > curr_jaw)
            
            # For short positions: Lips < Teeth < Jaw
            mouth_open_short = (curr_lips < curr_teeth < curr_jaw)
            
            # Calculate the "openness" of the mouth as the distance between Lips and Jaw
            mouth_openness = abs(curr_lips - curr_jaw)
            ...
            if prev_position == 1:
                # Exit long if price crosses below Lips
                price_cross_below_lips = (prev_price > prev_lips and curr_price < curr_lips)
                
                # Or if mouth is closing (Lips and Jaw converging)
                prev_openness = abs(sim_data.iloc[i-1][f'Lips_{strategy_name}'] - sim_data.iloc[i-1][f'Jaw_{strategy_name}'])
                mouth_closing = mouth_openness < prev_openness * 0.8  # 20% decrease in mouth openness
                
                if price_cross_below_lips or mouth_closing:
            ...
            # Exit conditions for short positions
                ...
                # Exit short if price crosses above Lips
                price_cross_above_lips = (prev_price < prev_lips and curr_price > curr_lips)
                
                # Or if mouth is closing (Lips and Jaw converging)
                prev_openness = abs(sim_data.iloc[i-1][f'Lips_{strategy_name}'] - sim_data.iloc[i-1][f'Jaw_{strategy_name}'])
                mouth_closing = mouth_openness < prev_openness * 0.8  # 20% decrease in mouth openness
                
                if price_cross_above_lips or mouth_closing:
                    sim_data.iloc[i, sim_data.columns.get_loc(f'Position_{strategy_name}')] = 0
                    sim_data.iloc[i, sim_data.columns.get_loc(f'Signal_{strategy_name}')] = 0
                    sim_data.iloc[i, sim_data.columns.get_loc(f'Entry_Price_{strategy_name}')] = np.nan
            ...
            if sim_data.iloc[i][f'Position_{strategy_name}'] == 0:
                ...
                # 1. Price crosses above Lips line
                price_cross_above_lips = (prev_price < prev_lips and curr_price > curr_lips)
                
                # 2. Alligator mouth is open properly (Lips > Teeth > Jaw)
                if price_cross_above_lips and mouth_open_long:
                ...
                # 1. Price crosses below Lips line
                price_cross_below_lips = (prev_price > prev_lips and curr_price < curr_lips)
                
                # 2. Alligator mouth is open properly (Lips < Teeth < Jaw)
                if price_cross_below_lips and mouth_open_short:
